---
id: version-3.3.0_Chinese-installation
title: Installation
...
您可以通过`singa==<version>`安装特定版本的SINGA，其中`<version>`字段应被替换，例如`3.3.0`。可用的SINGA版本在链接中列出。
...
您也可以配置SINGA版本和CUDA版本，比如`singa==3.3.0+cuda10.2`，SINGA版本和CUDA版本的可用组合在链接中列出。
